...
**A Universal GUI tool for detecting and organizing telomeric sequences in any _de novo_ assembled genome**
...
[![Tk](https://img.shields.io/badge/GUI-Tkinter-orange.svg)](#-installation-guide)
...
- **Forward** ITSs are moved to the **5′ start** of the scaffold.
- **Reverse** ITSs are moved to the **3′ end** of the scaffold.
...
│  • Region within `margin` bp of 5' end  → terminal (start)                  │
│  • Region within `margin` bp of 3' end  → terminal (end)                    │
...
Start-terminal — if its start coordinate is ≤ margin bp from the 5′ end.

End-terminal — if its end coordinate is ≤ margin bp from the 3′ end.
...
Detect terminal telomeres	✅	✅
Detect internal ITSs	✅	✅
Modify sequences	❌	✅
Write output FASTA	❌	✅
Write report	❌	✅
Print movements to log	✅	✅
When to use it
...
Sorts hits by start position, then by length (longest first).

Merges any hit that overlaps with an already-accepted region.

Keeps the longest region per locus.
...
Extracts the first and last 2000 bp of every scaffold (or the whole scaffold if shorter).

Counts occurrences of tandem arrays (≥ 3 repeats) for each preset's motifs.

Ranks the presets by total count and selects the best match.

Updates the GUI dropdown and logs the top 3 candidates with their scores.
...
It scans the first and last 2000 bp of every scaffold, counts tandem arrays for each preset's motifs, and picks the preset with the highest count. See Motif Suggestion for details and caveats.
